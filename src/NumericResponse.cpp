...
/*   Updated: 2024/09/12 18:48:55 by lhojoon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace irc {
    ...
    // 002
        RPLYourHost::RPLYourHost(std::string hostname, std::string version) {
            this->setCode("RPL_YOURHOST");
            this->setNumeric(2);
            std::stringstream ss;
            ss << ":Your host is " << hostname << ", running version " << version;
            this->setMessage(ss.str());
        }

    // 003
        RPLCreated::RPLCreated(std::time_t * time) {
            this->setCode("RPL_CREATED");
            this->setNumeric(3);
            std::tm * now = std::localtime(time);
            std::stringstream ss;
            ss << ":This server was created "
            << (now->tm_year + 1900) << '-' 
            << (now->tm_mon + 1) << '-'
